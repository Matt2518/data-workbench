...
window.DWBDashboard = (function() {
  ...
  function renderBarVertical(viz, rows, container) {
    ...
    chart.on('click', function(params) {
      if (!viz.linkToDisplayFilters || !viz.filterableFields || !viz.filterableFields.length) return;
      var field = catF; if (!field) return;
      var val = params.name || ''; if (!val) return;
      if (activeFilters[field] === val) { delete activeFilters[field]; } else { activeFilters[field] = val; }
      renderBanner(); renderChips(); renderPlacements();
    });
    setTimeout(function(){chart.resize();},50);
  }
